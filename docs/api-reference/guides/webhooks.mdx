...
<Note>
  Open Wearables deduplicates identical timeseries payloads. If a provider later corrects a sample at the same timestamp, the changed payload receives a new event ID and is delivered again with the corrected value.
</Note>
